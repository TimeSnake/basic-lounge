  ...
  public void init() {
    ...
    if (LoungeServer.getGame().getDiscordType().equals(Type.Discord.TEAMS)) {
      ...
      for (User user : LoungeServer.getUsers()) {
        List<UUID> uuids = uuidsByTeam.computeIfAbsent(
            ((LoungeUser) user).getTeam().getDisplayName(),
            k -> new LinkedList<>());
      ...
      uuidsByTeam.put(LoungeServer.DISCORD_SPECTATOR, List.of());
      uuidsByTeam.put(LoungeServer.DISCORD_LOUNGE,
          LoungeServer.getUsers().stream().map(User::getUniqueId).toList());
